...
function fullpagemaker(){
	...
	for (var i=0;i<root.total-1;i++){
		...
		$(f[0]).html("<span>"+"随便看看"+"</span>");
		if (i+2<root.total){
			for (var j=0;j<tmp.total;j++){
				$(f[j+1]).html("<span>"+allcon[tmp.all[j]].title+"</span>")
			}
		}
	}
...
function viewstart(){
	...
	//$("#logodiv").append("<div id='logo"+classSuffix+"' class='logo' style='font-size:2em; float:right;'>LM&nbsp;share</div>");
	$("#content").append("<div class='section' id='section_0'>"+
								"<div class='mid' id='level1'></div>"+
							"</div>");
	$("#level1").append("<div class='level1_out"+classSuffix+"' id='content_mid'></div>");
	var target = $("#content_mid");
	var i = root.total-1;
	$("#section").append("<div id='header_"+root.all[i]+"' class='sectioninner'  onclick='$.fn.fullpage.moveTo("+1+")'>"+allcon[root.all[i]].title+"</div>");
	for (i=0;i<root.total;i++) {
		if (root.all[i]!=alltot-1){
			$("#section").append("<div id='header_"+root.all[i]+"' class='sectioninner'  onclick='$.fn.fullpage.moveTo("+(i+2)+")'>"+allcon[root.all[i]].title+"</div>");
			target.append("<span id='" + root.all[i]+ "' class='level1_in"+classSuffix+"' onclick='$.fn.fullpage.moveTo("+(i+2)+")'><span>"+ allcon[root.all[i]].title +"</span></span>");
